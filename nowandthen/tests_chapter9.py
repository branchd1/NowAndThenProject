...
#
# In order to run these tests, copy this module to your tango_with_django_project/nowandthen/ directory.
# Once this is complete, run $ python manage.py test nowandthen.tests_chapter9
# 
...
import inspect
import tempfile
import nowandthen.models
from nowandthen import forms
from populate_nowandthen import populate
...
class Chapter9ModelTests(TestCase):
    ...
    def test_userprofile_class(self):
        """
        Does the UserProfile class exist in nowandthen.models? If so, are all the required attributes present?
        Assertion fails if we can't assign values to all the fields required (i.e. one or more missing).
        """
        self.assertTrue('UserProfile' in dir(nowandthen.models))

        user_profile = nowandthen.models.UserProfile()
    ...
    def test_model_admin_interface_inclusion(self):
        ...
        response = self.client.get('/admin/nowandthen/userprofile/')
...
class Chapter9RegisterFormClassTests(TestCase):
    ...
    def test_user_form(self):
        ...
        """
        self.assertTrue('UserForm' in dir(forms), f"{FAILURE_HEADER}We couldn't find the UserForm class in nowandthen's forms.py module. Did you create it in the right place?{FAILURE_FOOTER}")
    ...
    def test_user_profile_form(self):
        ...
        """
        self.assertTrue('UserProfileForm' in dir(forms), f"{FAILURE_HEADER}We couldn't find the UserProfileForm class in nowandthen's forms.py module. Did you create it in the right place?{FAILURE_FOOTER}")
        
        user_profile_form = forms.UserProfileForm()
        self.assertEqual(type(user_profile_form.__dict__['instance']), nowandthen.models.UserProfile, f"{FAILURE_HEADER}Your UserProfileForm does not match up to the UserProfile model. Check your Meta definition of UserProfileForm and try again.{FAILURE_FOOTER}")
...
class Chapter9RegistrationTests(TestCase):
    ...
    def test_new_registration_view_exists(self):
        ...
        try:
            url = reverse('nowandthen:register')
        except:
            pass
        
        self.assertEqual(url, '/nowandthen/register/', f"{FAILURE_HEADER}Have you created the nowandthen:register URL mapping correctly? It should point to the new register() view, and have a URL of '/nowandthen/register/' Remember the first part of the URL (/nowandthen/) is handled by the project's urls.py module, and the second part (register/) is handled by the nowandthen app's urls.py module.{FAILURE_FOOTER}")
    ...
    def test_registration_template(self):
        ...
        """
        template_base_path = os.path.join(settings.TEMPLATE_DIR, 'nowandthen')
        template_path = os.path.join(template_base_path, 'register.html')
        self.assertTrue(os.path.exists(template_path), f"{FAILURE_HEADER}We couldn't find the 'register.html' template in the 'templates/nowandthen/' directory. Did you put it in the right place?{FAILURE_FOOTER}")
        ...
        request = self.client.get(reverse('nowandthen:register'))
        content = request.content.decode('utf-8')

        self.assertTrue(re.search(full_title_pattern, content), f"{FAILURE_HEADER}The <title> of the response for 'nowandthen:register' is not correct. Check your register.html template, and try again.{FAILURE_FOOTER}")
    ...
    def test_registration_get_response(self):
        ...
        self.assertTrue('<h1>Register for nowandthen</h1>' in content, f"{FAILURE_HEADER}We couldn't find the '<h1>Register for nowandthen</h1>' header tag in your register template. Did you follow the specification in the book to the letter?{FAILURE_FOOTER}")
        self.assertTrue('nowandthen says: <strong>register here!</strong>' in content, f"{FAILURE_HEADER}When loading the register view with a GET request, we didn't see the required 'nowandthen says: <strong>register here!</strong>'. Check your register.html template and try again.{FAILURE_FOOTER}")
        ...
        self.assertTrue('action="/nowandthen/register/"' in content, f"{FAILURE_HEADER}Is your <form> in register.html pointing to the correct URL for registering a user?{FAILURE_FOOTER}")
    ...
    def test_bad_registration_post_response(self):
        ...
        """
        request = self.client.post(reverse('nowandthen:register'))
        content = request.content.decode('utf-8')
    ...
    def test_good_form_creation(self):
        ...
        self.assertEqual(len(nowandthen.models.UserProfile.objects.all()), 1, f"{FAILURE_HEADER}We were expecting to see a UserProfile object created, but it didn't appear. Check your UserProfileForm implementation, and try again.{FAILURE_FOOTER}")
    ...
    def test_good_registration_post_response(self):
        ...
        request = self.client.post(reverse('nowandthen:register'), post_data)
        content = request.content.decode('utf-8')

        self.assertTrue('<h1>Register for nowandthen</h1>' in content, f"{FAILURE_HEADER}We were missing the '<h1>Register for nowandthen</h1>' header in the registration response.{FAILURE_FOOTER}")
        self.assertTrue('nowandthen says: <strong>thank you for registering!</strong>' in content, f"{FAILURE_HEADER}When a successful registration occurs, we couldn't find the expected success message. Check your implementation of register.html, and try again.{FAILURE_FOOTER}")
        self.assertTrue('<a href="/nowandthen/">Return to the homepage.</a>' in content, f"{FAILURE_HEADER}After successfully registering, we couldn't find the expected link back to the nowandthen homepage.{FAILURE_FOOTER}")
    ...
    def test_base_for_register_link(self):
        ...
        self.assertTrue('<li><a href="{% url \'nowandthen:register\' %}">Sign Up</a></li>' in template_str)
    

class Chapter9LoginTests(TestCase):
    """
    A series of tests for checking the login functionality of nowandthen.
    """
    def test_login_url_exists(self):
        ...
        try:
            url = reverse('nowandthen:login')
        except:
            pass
        
        self.assertEqual(url, '/nowandthen/login/', f"{FAILURE_HEADER}Have you created the nowandthen:login URL mapping correctly? It should point to the new login() view, and have a URL of '/nowandthen/login/' Remember the first part of the URL (/nowandthen/) is handled by the project's urls.py module, and the second part (login/) is handled by the nowandthen app's urls.py module.{FAILURE_FOOTER}")

    def test_login_functionality(self):
        """
        Tests the login functionality. A user should be able to log in, and should be redirected to the nowandthen homepage.
        """
        user_object = create_user_object()

        response = self.client.post(reverse('nowandthen:login'), {'username': 'testuser', 'password': 'testabc123'})
        ...
        self.assertEqual(response.url, reverse('nowandthen:index'), f"{FAILURE_HEADER}We were not redirected to the nowandthen homepage after logging in. Please check your login() view implementation, and try again.{FAILURE_FOOTER}")
    ...
    def test_login_template(self):
        ...
        self.assertTrue(os.path.exists(template_path), f"{FAILURE_HEADER}We couldn't find the 'login.html' template in the 'templates/nowandthen/' directory. Did you put it in the right place?{FAILURE_FOOTER}")

        template_str = get_template(template_path)
        full_title_pattern = r'<title>(\s*|\n*)nowandthen(\s*|\n*)-(\s*|\n*)Login(\s*|\n*)</title>'
        block_title_pattern = r'{% block title_block %}(\s*|\n*)Login(\s*|\n*){% (endblock|endblock title_block) %}'

        request = self.client.get(reverse('nowandthen:login'))
        content = request.content.decode('utf-8')

        self.assertTrue(re.search(full_title_pattern, content), f"{FAILURE_HEADER}The <title> of the response for 'nowandthen:login' is not correct. Check your login.html template, and try again.{FAILURE_FOOTER}")
    ...
    def test_login_template_content(self):
        ...
        template_str = get_template(template_path)
        self.assertTrue('<h1>Login to nowandthen</h1>' in template_str, f"{FAILURE_HEADER}We couldn't find the '<h1>Login to nowandthen</h1>' in the login.html template.{FAILURE_FOOTER}")
        self.assertTrue('action="{% url \'nowandthen:login\' %}"' in template_str, f"{FAILURE_HEADER}We couldn't find the url lookup for 'nowandthen:login' in your login.html <form>.{FAILURE_FOOTER}")
    ...
    def test_homepage_greeting(self):
        ...
        """
        content = self.client.get(reverse('nowandthen:index')).content.decode()
        self.assertTrue('hey there partner!' in content, f"{FAILURE_HEADER}We didn't see the generic greeting for a user not logged in on the nowandthen homepage. Please check your index.html template.{FAILURE_FOOTER}")
...
class Chapter9RestrictedAccessTests(TestCase):
    ...
    def test_restricted_url_exists(self):
        ...
        try:
            url = reverse('nowandthen:restricted')
        except:
            pass
        
        self.assertEqual(url, '/nowandthen/restricted/', f"{FAILURE_HEADER}Have you created the nowandthen:restricted URL mapping correctly? It should point to the new restricted() view, and have a URL of '/nowandthen/restricted/' Remember the first part of the URL (/nowandthen/) is handled by the project's urls.py module, and the second part (restricted/) is handled by the nowandthen app's urls.py module.{FAILURE_FOOTER}")
    ...
    def test_bad_request(self):
        ...
        """
        response = self.client.get(reverse('nowandthen:restricted'))
        ...
        self.assertTrue(response.url.startswith(reverse('nowandthen:login')), f"{FAILURE_HEADER}We tried to access the restricted view when not logged in, and were expecting to be redirected to the login view. But we were not! Please check your restricted() view.{FAILURE_FOOTER}")
...
class Chapter9LogoutTests(TestCase):
    ...
    def test_bad_request(self):
        ...
        """
        response = self.client.get(reverse('nowandthen:logout'))
        self.assertTrue(response.status_code, 302)
        self.assertTrue(response.url, reverse('nowandthen:login'))
    ...
    def test_good_request(self):
        ...
        self.assertEqual(response.url, reverse('nowandthen:index'), f"{FAILURE_HEADER}When logging out a user, the book states you should then redirect them to the homepage. This did not happen; please check your logout() view.{FAILURE_FOOTER}")
...
class Chapter9LinkTidyingTests(TestCase):
    ...
    def test_omnipresent_links(self):
        ...
        """
        content = self.client.get(reverse('nowandthen:index')).content.decode()
        self.assertTrue('href="/nowandthen/about/"' in content)
        self.assertTrue('href="/nowandthen/"' in content)
        ...
        content = self.client.get(reverse('nowandthen:index')).content.decode()
        self.assertTrue('href="/nowandthen/about/"' in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/nowandthen/"' in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
    ...
    def test_logged_in_links(self):
        ...
        # These should be present.
        self.assertTrue('href="/nowandthen/restricted/"' in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/nowandthen/logout/"' in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")

        # These should not be present.
        self.assertTrue('href="/nowandthen/login/"' not in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/nowandthen/register/"' not in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
    ...
    def test_logged_out_links(self):
        ...
        # These should be present.
        self.assertTrue('href="/nowandthen/login/"' in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/nowandthen/register/"' in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        
        # These should not be present.
        self.assertTrue('href="/nowandthen/restricted/"' not in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/nowandthen/logout/"' not in content, f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
...
class Chapter9ExerciseTests(TestCase):
    ...
    def test_restricted_template_exists(self):
        ...
        self.assertTrue(os.path.exists(template_path), f"{FAILURE_HEADER}We couldn't find the 'restricted.html' template in the 'templates/nowandthen/' directory. Did you put it in the right place? Did you complete the exercises?{FAILURE_FOOTER}")
    ...
    def test_restricted_template_inherits(self):
        ...
        user_object = create_user_object()
        self.client.login(username='testuser', password='testabc123')
        request = self.client.get(reverse('nowandthen:restricted'))
        content = request.content.decode('utf-8')

        self.assertTrue(re.search(full_title_pattern, content), f"{FAILURE_HEADER}The <title> of the response for 'nowandthen:restricted' is not correct. Check your restricted.html template, and try again.{FAILURE_FOOTER}")
    ...
    def test_bad_add_page(self):
        ...
        """
        populate()
        response = self.client.get(reverse('nowandthen:add_page', kwargs={'category_name_slug': 'python'}))
        ...
        self.assertTrue(response.url.startswith(reverse('nowandthen:login')), f"{FAILURE_HEADER}When not logged in and attempting to add a page, we should be redirected to the login page. But we weren't. Check your add_page() implementation.{FAILURE_FOOTER}")
    
    def test_bad_add_category(self):
        """
        Tests to see if a category cannot be added when not logged in.
        """
        response = self.client.get(reverse('nowandthen:add_category'))
        ...
        self.assertTrue(response.url.startswith(reverse('nowandthen:login')), f"{FAILURE_HEADER}When attempting to add a category when not logged in, we weren't redirected to the login view. Check your add_category() implementation, and try again.{FAILURE_FOOTER}")
    ...
    def test_add_category_link(self):
        ...
        self.assertTrue(reverse('nowandthen:add_category') not in content, f"{FAILURE_HEADER}The Add Category link was present on nowandthen's homepage when a user is not logged in. This shouldn't be the case! Please check your base.html template.{FAILURE_FOOTER}")

        user_object = create_user_object()
        self.client.login(username='testuser', password='testabc123')
        content = self.client.get(reverse('nowandthen:index')).content.decode()

        self.assertTrue(reverse('nowandthen:add_category') in content, f"{FAILURE_HEADER}The Add Category link was not present on nowandthen's homepage when the user was logged in. It should be visible! Please check base.html.{FAILURE_FOOTER}")
    ...
    def test_add_page_link(self):
        ...
        """
        populate()
        content = self.client.get(reverse('nowandthen:show_category', kwargs={'category_name_slug': 'python'})).content.decode()
        
        self.assertTrue(reverse('nowandthen:add_page', kwargs={'category_name_slug': 'python'}) not in content, f"{FAILURE_HEADER}The Add Page link was present in the show_category() response when a user was not logged in. It shouldn't be there. Did you do the exercises?{FAILURE_FOOTER}")

        user_object = create_user_object()
        self.client.login(username='testuser', password='testabc123')
        content = self.client.get(reverse('nowandthen:show_category', kwargs={'category_name_slug': 'python'})).content.decode()

        self.assertTrue(reverse('nowandthen:add_page', kwargs={'category_name_slug': 'python'}) in content, f"{FAILURE_HEADER}The Add Page link was not present when a user was logged in, and looking at the show_category() view. Did you make a mistake in your category.html template?{FAILURE_FOOTER}")
